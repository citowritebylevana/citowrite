...
export default function PricingCard({
  ...
  const handleCardClick = () => {
    router.push(`/order?service=${encodeURIComponent(title)}`);
  };

  return (
    <div
      onClick={handleCardClick}
      className={`flex flex-col p-5 md:p-6 rounded-[20px] transition-all ease-out duration-300 h-full cursor-pointer
      ${
        isHighlighted
    ...
    >
      {/* Title */}
      <h3
        className={`text-[28px]/9.5 font-semibold mb-2.5 ${isHighlighted ? "text-indigo-400" : "text-white"}`}
      >
        {title}
      </h3>

      {/* Description */}
      <p className="text-white text-base/6 mb-10">{description}</p>
      ...
      <ul className="space-y-1.5 mb-10 grow">
        {features.map((feature, idx) => (
          <li
            key={idx}
            className="flex items-start gap-2 md:gap-3 text-indigo-50 text-sm"
          >
